// Scenario Builder Chart Fix - Updated with proper data sorting
document.addEventListener('DOMContentLoaded', function() {
    console.log("Scenario builder script loaded");
    
    // Generate scenario data function
    function generateScenarioData() {
        console.log("Generating scenario data");
        ...
        // Create dates array first to ensure proper sorting
        const dates = [];
        ...
        // Generate historical data with sorted dates
        ...
        // Generate projection data with sorted dates
        ...
        console.log("Bullish data points: " + bullish.length);
        console.log("Base data points: " + base.length);
        console.log("Bearish data points: " + bearish.length);
        console.log("Range data points: " + range.length);
    ...
    if (typeof Highcharts === 'undefined') {
        console.error('Highcharts library not found. Please include Highcharts before this script.');
        return;
    }

    // Check if the scenario-builder-chart element exists (using the correct ID from HTML)
    const chartContainer = document.getElementById('scenario-builder-chart');
    if (chartContainer) {
        console.log("Scenario builder chart container found");
        ...
        try {
            ...
            if (controlsContainer) {
                console.log("Scenario controls container found");
                
                // Create the controls if they don't exist
                if (controlsContainer.children.length === 0) {
                    controlsContainer.innerHTML = `
                        <div class="control-group">
                            <label for="clinical-success">Clinical Trial Success Probability:</label>
                ...
                if (clinicalSuccessSlider && marketSentimentSlider && cashRunwaySlider) {
                    console.log("Scenario control sliders found");
                    ...
                    console.error("Could not find all scenario control sliders");
                }
            } else {
                console.error("Scenario controls container not found");
            }
        } catch (error) {
            console.error("Error creating scenario chart:", error);
            // Add a visible error message to the chart container
            chartContainer.innerHTML = `
                <div style="padding: 20px; background-color: #f8d7da; color: #721c24; border-radius: 8px;">
                    <h3>Chart Error</h3>
                    <p>There was an error rendering the scenario chart: ${error.message}</p>
                    <p>Please check the console for more details.</p>
                </div>
            `;
        }
    } else {
        console.error('Scenario builder chart container not found (ID: scenario-builder-chart)');
    }
});
